...
cities_html <- read_html(______)
# Parse Source Code
tables <- html_nodes(_______, "table.wikitable")
...
for (p in __:__){
  ...
  ## Taking the root and modifying it to iterative over each page, then read_html
  obama <- _____(____(_____, "briefing-room/speeches-and-remarks?term_node_tid_depth=31&page=", 
                           p, sep = ""))
  ...
  for(i in _:__){
    ...
    date_nodes <- ______(link_app, css="__________")
    
    ## Pulling the text from the node 
    date <- html_text(_____)
...
```{r}
# Create corpus
speeches_corpus <- _____::____(____, docid_field ="___", docnames = docnames, text_field ="_____")
# View Object
...
Manually create tokens using `tokens()` removing numbers and punctuation. Make sure to specify ngrams = 1. Examine the resulting object using `head()`
```{r}
new_tokens <- tokens(________,
                     ________,
                     ________,
                     ________)
...
new_tokens2 <- sapply(_______, function(x) ifelse(__ %in% ______("_____"), "", ___))
```
...
eng.stopwords <- __________
keep.stopword <- ______[-c(which(_______=="___"))]
# Specify America as a stopword
keep.stopword <- c("______",______)
...
speeches_corpus <- __________(______, docid_field ="id", docnames = docnames, text_field ="text")

# Create DFM from speeches_corpus
speeches_dfm <- dfm(_____, 
                    _____,
                    _____, 
                    _____,
                    _____)
...
ggplot(______, aes(x=_____, y=_____, fill=______)) +
