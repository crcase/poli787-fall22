...
library(dplyr)
nes92 <- nesdata %>% filter(year == 1992)
...
nes92 <- nes92  %>% 
  mutate_at(vars(gender, race, urban, educ1), .funs = as.factor)

# Estimate Model
m1 <- glm(rep_presvote ~ partyid7 + gender + race + urban + educ1, 
...
roc.vote <- roc(predict(m1, type="response"),
                 as.factor(model.frame(m1)$rep_presvote))
# Compute AUC-ROC
round(auc(roc.vote),2)
...
sim.pid <- seq(min(nes92$partyid7, na.rm = TRUE), 
               max(nes92$partyid7, na.rm = TRUE), by = 1)

# Create Prediction Data
pred.data <- nes92
# Create Plot Data DF with 4 columns and 7 rows
plot.data <- data.frame(matrix(ncol = 4, nrow = 7))
# Rename columns 
...
for (i in sim.pid){
  pred.data$partyid7 <- i # Replace partyid7 in pred.data
  all.preds <- predict(m1, # Obtain Predictions with Standard Errors
                       newdata = pred.data,
                       type = 'response',
                       se.fit = TRUE)
  plot.data$pid[i] <- i # Save PID value in plot.data
  plot.data$preds[i] <- mean(all.preds$fit, na.rm = TRUE) # Save predictions
  plot.data$LB[i] <- mean(all.preds$fit - qnorm(0.975) * all.preds$se.fit,
                          na.rm = TRUE) # Save 95% CI LB
  plot.data$UB[i] <-  mean(all.preds$fit + qnorm(0.975) * all.preds$se.fit,
                          na.rm = TRUE) # Save 95% CI UB
}
  
# Create Plot for Predicted Probability of Voting for Bush
library(ggplot2)
ggplot(plot.data, aes(y = preds, x = pid)) +
  geom_line() +
  geom_pointrange(aes(ymin=LB, ymax=UB)) +
  theme_bw() +
  labs(x = 'Partisan Identification',
       y = 'Predicted Probability of Voting for Bush')
...
# Run Null Model with COuntry RE
nullmod <- lmer(as.numeric(imwbcnt) ~ (1|cntry), dat)

# Variance Structure
#Find variance attributed to country
interceptvar <- VarCorr(nullmod)$cntry %>% as.numeric() 
# Find the residual varriance
residualvar <- attr(VarCorr(nullmod), "sc")^2 
...
For the random slope, you can plot the correlation by country plots using ggplot (You can also do this using `xyplot()` from the `Hmisc` package where you can specify the exact model). Plot the correlation between left-right scale and the dependent variable to see if there's country-level effects varying the slope of `lrscale`.

```{r}
ggplot(dat, aes(x = imwbcnt, y = lrscale)) + # Data frame and X/Y arguments
  stat_smooth(method = "lm", se = F) +# Create loess line, se = F
  facet_wrap(cntry ~ ., ncol = 6) + # Arrange individual plots for each country
  theme_bw()
...
m1 <- lmer(imwbcnt ~ lrscale + stflife+ trstplt+ rlgdgr+ as.factor(gndr) 
           + age+ (1|cntry), data =dat)
# Random Slope Model
m2 <- lmer(imwbcnt ~ lrscale + stflife+ trstplt+ rlgdgr+ as.factor(gndr) 
           + age + (1 + lrscale|cntry), data =dat)
```
...
ggplot(dat2, aes (x = lrscale, y = fit, colour = cntry, group = cntry)) + 
  geom_point(position = "jitter", alpha = .05) +
  geom_smooth(method = "lm", se = F) +
  geom_smooth(data = dat2, aes(x = lrscale, y = fit), 
