...
beta <- matrix(colMeans(science2), nrow=J, ncol=1, byrow=TRUE) # colmeans
x <- matrix(rowMeans(science2), nrow=N, ncol=1, byrow=TRUE) # Rowmeans
...
# Create Priors
x_mu <- matrix(0, nrow=1, ncol=1, byrow=TRUE) # Zero 
x_sigma <- matrix(1, nrow=1, ncol=1, byrow=TRUE) # one
...
xpriors <- list(mu = x_mu, sigma= x_sigma)
betapriors <- list(mu = beta_mu, sigma= beta_sigma)

priors <- list(x=xpriors,beta=betapriors)

# Fit ordinal IRT
emfit <- ordIRT(science2,
                .starts = starts,
                .priors = priors,
                .D=1,
...
priors <- makePriors(nrow(im), ncol(im), 1)
starting <- getStarts(nrow(im), ncol(im), 1)
# Get roll call votes
rc <- list(votes = im)
# Estimate binIRT
fit <- binIRT(.rc = rc,
...
# extract empirical 95% CI
results <- data.frame(id = seq(1:nrow(im)),
                      idealpt = unname(fit$means$x),
                      ci_lwr = sapply(fit$means$x, function(x) (x - (1.96*sqrt(fit$vars$x)))), #why do I need to use sapply?
                      ci_upr = sapply(fit$means$x, function(x) (x + (1.96*sqrt(fit$vars$x)))))

### randomly sample 500 people
results <- sample_n(results, 500 ,replace=FALSE)

#sort df by ideal point est.
results <- results %>%
  arrange(idealpt) %>%
  mutate(order = seq(from=1,to=nrow(results)))

# plot ideal points and uncertainty
ggplot(results, aes(y = idealpt, x = order, ymin = ci_lwr,
                           ymax = ci_upr)) +
  geom_point(aes(), cex = .25,) +
  geom_linerange(size = .15, col = 'grey', alpha=0.5) +
  coord_flip() +
  labs(title="Estimating Attitudes to Immigration", y = "**scale unclear**")+
  theme_bw() +
  theme(plot.background = element_blank(),
        panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
