...
data$trimmed <- gsub('_____', '', data$trimmed)
data$trimmed <- gsub("_____", " ", data$trimmed)

## drop any observations with no text
data <- subset(data, _____________)

## create corpus object, set text field
issues <- corpus(data, text_field = "_______")

## finish preprocessing steps (stop words (including Congress), stem and remove punctuation) 
eng.stopwords <- __________________________
eng.stopwords <- c("_______", _______)
iss_dfm <- dfm(issues, 
               _______,
               _______, 
               _______
...
iss_lda <- convert(______, to = '____')
iss_stm <- convert(______, to = '____')
...
iss_lda <- prepDocuments(iss_lda$_______, iss_lda$______,
                         iss_lda$meta, lower.thresh = 5)

iss_stm <- prepDocuments(iss_stm$_______, iss_stm$_____,
...
initial_lda <- stm(documents = ___________, 
                   vocab = ___________,
                   K = __, 
                   data = iss_lda$meta, 
                   init.type = "Spectral")
...
semcoh <- semanticCoherence(_________, documents = iss_stm$documents)
exclusivity <- exclusivity(_________)

semco <- data.frame(______, ______, ________)
...
initial_model <- ________________
```
...
prep <- estimateEffect(___ ~ ______, ___________, meta = iss_stm$meta)

plot(_____, 
     covariate="_____", 
     c(1:9),
     model=_________, method="difference",
...
keyATM_docs <- keyATM_read(texts = _______)
summary(keyATM_docs)
...
_____________(docs = __________, keywords = _________, label_size = 5)
...
out <- keyATM(docs              = ________,
              no_keyword_topics = ________,
              keywords          = ________,
              model             = "________",
              model_settings    = list(covariates_data = ________,
                                       covariates_formula = ~ ________________),
...
strata_topic <- by_strata_DocTopic(out, by_var = "_______",
                                   labels = c("Male", "Female"))
fig_doctopic <- plot(strata_topic, var_name = "______", show_topic = c(1:5))
fig_doctopic
