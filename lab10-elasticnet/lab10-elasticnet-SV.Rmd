...
x <- matrix(rnorm(n * p), nrow = _, ncol = _)
# first 15 predictors affect outcome
y <- apply(x[, __:___], 1, sum) + rnorm(n) 
```
...
train_rows <- sample(_:_, ___*___)
# Split predictors into training, test data
x_train <- x[__________, ]
x_test <- x[-__________, ]
# Split outcome into training, test data
y_train <- y[____________]
y_test <- y[-____________]
```

Now, there are two parameters to tune: $\lambda$ and $\alpha$. `glmnet` allows use to tune $\lambda$ via cross validation, but only with a fixed $\alpha$, so we are going to focus on just three: $\alpha = \{0,.5, 1\}$. Set family equal to gaussian.



```{r, message = F}
# Fit models using variying alpha levels
fit_lasso <- ______(______, _______, family = '________', alpha=___) # LASSO
fit_ridge <- ______(______, _______, family = '________', alpha=___) # ridge
fit_elnet <- ______(______, _______, family = '________', alpha=___) # e-net
```
...
fits <- mclapply(0:10, function(x) cv.glmnet(______, ______, type.measure='____',
...
Our lasso picked all 15 of the predictors we used to create our response variable -- nice! It also picked other predictors that *weren't* in our model, but notice that the estimated coefficients for them (mean = 0) are much smaller than for the 'true' variables (mean = 0.92). This suggests that our lasso did a good job identifying important features because the implicit coefficients on the first 15 predictors are equal to 1, while the absence of the remaining predictors means their implicit coefficients are equal to 0.
...
```{r, fig.align = 'center'}
# fit a glmnet with our best alpha value of 1
fit_best <- _____(_____, ______, family = 'gaussian', alpha = ____)
...
# sequences of alpha values to evaluate (by .1)
alphas <- ___________________

# cross validation elastic nets for different penalty parameters
fits <- mclapply(_____, function(x) _____(____, ____, type.measure = '____',
                                            alpha = ___, family = '________'))

# plot AUC for alpha = 0, .5, and 1
par(mfrow = c(3,1))
plot(fits[[___]])
plot(fits[[___]])
plot(fits[[___]])
...
best_fit <- glmnet(___, ____, family = '______', alpha = __________)
...
betas <- ______________________________________________

# sample 10 observations for each outcome
fl_samp <- ______________________________________________

# create predictors and response for groups
fl_x_samp <- __________
fl_y_samp <- __________

# fit sample data w/ best penalty parameter from entire dataset
best_fit_samp <- glmnet(______, _______, family = '______', alpha =________)
...
betas_samp <- ______________________________________________
