...
bal.tab(treat ~ __ + ___ + _____ + ____ + ___ + ___,
        data = _____, estimand = "ATT", m.threshold = .05)
...
ps.mod <- ___(treat~ age + educ + married + nodegree + re74 + re75, 
              data = lalonde, family = "binomial")
# Create predictions
ps <- predict(____, type = "response")
...
lalonde$weights <- ifelse(lalonde$treat == 1, ___, # w=1 for treatment
                  ________ # propensity score/(1-propensity score)
```
...
_____(____, m.threshold = .05, disp.v.ratio = TRUE)
```
...
# Plot Weights
ggplot(wdf, aes(x=____, y=_______))+
  geom_point()+
...
```{r}
# Load data
ncp_final <- readRDS(________)

# Look at balance of demographic variables
______(______ ~ gender + region + edu + yob, 
        data = _____, estimand = ___, m.threshold = .05)
```
...
# Estimate model
tmod2 <- svyglm(___________________________ 
                design = design)
# 95% CI
...
ncp_clean <- ________(______)

# Calculate weights with survey weights (specify s.weights)
W.out <- weightit(__________________________, 
                  data = _______, estimand = "ATT", method = "ps",
                  _______= _______)
# Check balance
...
att.weights <- ___________ *____________ #product of weights
...
5. Report all models in the same table using stargazer (make sure to specify html document above). What is your conclusion?

```{r results="asis"}
#Putting it all together:

stargazer(____, ____, ____, type="html",
          _____________ = c("Primed",
                               "Political Interest",
                               "Female",
                               "Østlandet",
                               "Sørlandet",
